...
| 控制推理中的内存开销 | 复用张量及临时图像缓冲，前处理直接写入输入张量 | 颜色转换、越界填充和公共结果生成仍可能分配或复制内存 |
...
BGR、RGB 和 GRAY 使用图像行跨度。NV12 / NV21 使用紧凑连续平面，先缩放 Y 和色度平面再转换颜色；需要二阶段裁剪时，对原始 YUV 的局部区域进行转换。固定 ROI 和裁剪后的结果通过偏移恢复原图坐标。
...
公共 `AlgResult` 使用三个独立的强类型数组，应用无需访问内部属性或 `field_mask`。车牌文本最多输出 31 字节并以空字符结束；空文本可保留为结果，应用应另行判断业务有效性。
...
2. `AlgRun` 检查顶层指针，释放传入结果中的旧数组。
3. 检测阶段完成图像前处理、模型推理、检测解码和 NMS。
4. 识别阶段对 PARE 执行透传，对其他候选框裁剪后执行门控与 OCR。
5. 识别结果写回检测框，并执行联合分及最小框过滤。
6. 汇总有效对象，分配和填充公共数组，设置成功调用帧号。
7. 应用消费结果，通过 `AlgFreeResult` 释放数组，结束时调用 `AlgDestroy`。
...
| 公共结果数组 | SDK 分配、应用管理生命周期 | `AlgFreeResult` 或同一结果结构的下一次有效 `AlgRun` |

首次使用结果结构必须零初始化。`AlgDestroy` 不释放已返回的公共结果，结果浅拷贝也不构成独立所有权。
...
如果模型产生新的公共业务结果，应一并扩展 `alg_types.h`、`FillAlgResult`、`AlgFreeResult`、示例程序和接口文档。
...
- [接口文档](alg_sdk_api_documentation_v1.0.0.md)：公共 ABI、调用约束和错误码。
